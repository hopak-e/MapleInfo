...
interface CharVMatrixProps {
  ocid: string;
}

const CharVmatrix = ({ ocid }: CharVMatrixProps) => {
  const [vMatrix, setVMatrix] = useState<Skill[]>();
  const [hoveredSkill, setHoveredSkill] = useState<Skill | null>(null);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const hexa: Matrix = await CharApiService.fetchHexa(ocid, 5);
        setVMatrix(hexa.character_skill);
      } catch (error) {
